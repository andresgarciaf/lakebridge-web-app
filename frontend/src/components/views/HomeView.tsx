...
export function HomeView({ onNavigate }: { onNavigate: (v: View) => void }) {
  return (
    <div className="h-full flex flex-col">
      <section className="flex-1 min-h-0 flex flex-col items-center justify-center text-center px-6 py-6">
        <LakebridgeLogo height={72} variant="lockup" />
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-5">
          {CARDS.map((card) => (
            <button
              ...
              className="text-left bg-[#ecebe8] hover:bg-[#e2e1de] active:bg-[#d8d7d4] transition rounded-xl p-6 flex flex-col gap-4 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FF3621]"
            >
              <div>{card.icon()}</div>
              <div>
                <h2 className="text-lg font-semibold text-slate-900">{card.title}</h2>
                <p className="mt-2 text-slate-700 leading-relaxed text-sm">{card.description}</p>
              </div>
            </button>
          ))}
        </div>
